...
// ── Fallback images shown when the API returns nothing ───────
const FALLBACK_STICKERS: Pick<Sticker, 'imageUrl' | 'category'>[] = [
...
export default function LandingPage() {
  const [headerAnimation, setHeaderAnimation] = useState<AnimationType>('fade');

  useEffect(() => {
    api.get<{ headerAnimation?: AnimationType }>('/settings')
      .then((res) => {
      ...
      .catch(() => {
        // Non-critical; keep the default 'fade' animation
      });
  }, []);
...
// ─────────────────────────────────────────────────────────────
function Header({ animationType }: { animationType: AnimationType }) {
  ...
  const { initial, animate, transition } = variants[animationType];
      ...
      <div className="flex justify-between items-center w-full px-container-padding py-4 max-w-7xl mx-auto">
        ...
        <nav className="hidden md:flex gap-8 items-center">
          {['Services', 'Gallery', 'Contact'].map((label) => (
            <a
              key={label}
              className="text-on-secondary-container hover:text-primary transition-colors font-semibold"
        ...
        <div className="flex items-center gap-4">
          <span className="text-on-secondary-container text-sm font-bold hidden lg:block font-display">
            98400 77303
          </span>
          <a
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-grid-gutter">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-display font-bold leading-tight">
            Expert Signage & <br className="hidden lg:block" />
            Custom Sticker Solutions in Chennai
          </h1>
          <p className="text-lg text-on-primary-container max-w-2xl leading-relaxed">
            Professional, high-quality, and durable printing services for your business and vehicle needs.
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-on-surface-variant max-w-2xl mx-auto">
            Comprehensive printing and signage solutions tailored for maximum impact and durability.
          </p>
        </div>
          ...
          {serviceCategories.map((service, idx) => (
            ...
            >
              ...
              <div className="p-8 relative">
                {!service.image && <div className="absolute top-0 inset-x-0 h-1 bg-primary" />}
...
// ─────────────────────────────────────────────────────────────
function Portfolio() {
  const [stickers, setStickers] = useState<Pick<Sticker, 'imageUrl' | 'category' | 'title'>[]>([]);

  useEffect(() => {
    api.get<Sticker[]>('/stickers')
      .then((res) => {
        setStickers(res.data.length > 0 ? res.data : FALLBACK_STICKERS as Sticker[]);
      })
      .catch(() => {
        setStickers(FALLBACK_STICKERS as Sticker[]);
      });
  }, []);
    ...
    <section className="py-section-gap px-container-padding bg-surface" id="gallery">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-on-surface-variant max-w-2xl mx-auto">
            A selection of our precision-crafted signage and sticker projects across Chennai.
          </p>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stickers.map((img, idx) => (
            ...
            >
              <img
                alt={`${img.category || 'Portfolio'} example ${idx + 1}`}
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-grid-gutter">
        ...
        <div>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3 text-on-primary-container">
